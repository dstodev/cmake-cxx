...
function(balanced_match out_var left_delimiter right_delimiter in_str)
	...
	string(LENGTH "${right_delimiter}" right_len)
	string(LENGTH "${in_str}" in_len)

	find_substrings(lefts "${left_delimiter}" "${in_str}")
	find_substrings(rights "${right_delimiter}" "${in_str}")
...
function(test_balanced_match_nested)
	set(input "some(t(hi)ng)else")
	balanced_match(out "(" ")" "${input}")
	expect(DEFINED out)
	expect("${out}" STREQUAL "t(hi)ng")
endfunction()
